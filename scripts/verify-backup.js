/**
 * X-29 (x-2k-29) Read-Only Firestore Backup Deep Verification System
...
const BACKUP_BASE_DIR = path.join(X29_ROOT_DIR, 'X-29-backup');
...
function scanAvailableBackups() {
    ...
    for (const { fullPath, jsonPath } of backupDirs) {
        ...
        if (relPath.startsWith('Manual') || relPath.startsWith('manual') || relPath.startsWith('Manual' + path.sep) || relPath.startsWith('manual' + path.sep)) {
            ...
            relSource = 'X-29-backup\\Automatic';
        }
